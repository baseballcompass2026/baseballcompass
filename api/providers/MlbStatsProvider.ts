...
// MLB公式Stats APIからキー不要で当日情報を取得するProvider
export class MlbStatsProvider implements BaseballProvider {
  ...
  private async games(date: Date) {
    const url=`https://statsapi.mlb.com/api/v1/schedule?sportId=1&date=${encodeURIComponent(this.formatDate(date))}&hydrate=team,linescore`;
    const response=await fetch(url,{headers:{Accept:"application/json","User-Agent":"BASEBALL-COMPASS/1.0"},cf:{cacheTtl:900,cacheEverything:true}});
    if(!response.ok) throw new Error(`MLB Stats API: ${response.status}`);
    const json=await response.json() as MlbSchedule;
    return (json.dates?.flatMap(item=>item.games)||[]).map(game=>{
      const played=game.status.abstractGameState==="Final"||game.status.abstractGameState==="Live";
      return {away:game.teams.away.team.name,home:game.teams.home.team.name,time:new Intl.DateTimeFormat("ja-JP",{timeZone:"Asia/Tokyo",hour:"2-digit",minute:"2-digit",hour12:false}).format(new Date(game.gameDate)),venue:game.venue?.name||"会場未定",status:game.status.detailedState,score:played?`${game.teams.away.score??0} - ${game.teams.home.score??0}`:undefined};
    });
  }
  ...
  async getRanking() { return {standings:[],homeRuns:[],batting:[],era:[]}; }
